        ...
        for assert_key, assert_value in value.items():
            if assert_key == "msg_code":
                ...
                if isinstance(resp_list[0], str):
                    resp_list = ''.join(resp_list) # 列表转换为字符串
                if resp_list:
                    assert_value = None if assert_value.upper() == 'NONE' else assert_value # 处理yaml文件中None值，转换为python能够识别None的类型
...
if __name__ == '__main__':
    ...
    assertion.assert_contains(1,'<UNK>','200')
